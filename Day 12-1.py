...
class HeightMap:
    ...
    def find_path_to_end(self):
        found_exit = False
        to_visit_queue = [self.start_point]
        ...
        if found_exit == True:
            print("the shortest number of steps to the goal is " + str(shortest_coords["steps"]))
        else:
            print("ERROR: There is no path from the start point to an end point.")

jungle = HeightMap()
jungle.create_map("Data Files/Day 12 Data.txt")
# jungle.print_map()
jungle.find_path_to_end()